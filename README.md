...
- [anchor-x402](https://anchor-x402.com) - 16 x402 pay-per-call services + MCP server. Nine commodity primitives — wallet sanctions/AML screening, bundled wallet intelligence (balances + activity + identity + risk), cross-chain name resolution (ENS + SNS), USD token prices, transaction & calldata decoding, datetime parsing, signed decision attestation, dual-chain hash anchoring on Base + Solana mainnet. Plus an async wallet due-diligence investigator ($7.77, signed report + on-chain anchor), a verifiable signed RNG, and 5 universal LLM endpoints (roast, anchored yes/no oracle, tldr, aura, grade). No API keys; USDC per call.
